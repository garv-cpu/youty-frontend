...
function App() {
    ...
    useEffect(() => {
        const fetchMetadata = async () => {
            if (!videoId) return;
            try {
                const response = await fetch(`https://youty-backend.onrender.com/metadata?url=${encodeURIComponent(url)}`);
    ...
    const handleDownload = async () => {
        ...
        try {
            const response = await fetch(`https://youty-backend.onrender.com/download?url=${encodeURIComponent(url)}`);
            ...
            <div className="bg-[#192734] shadow-xl rounded-2xl p-6 w-full max-w-md text-white">
                ...
                {videoId && (
                    <>
                        ...
                        {metadata && (
                            <div className="mb-4">
                                <h2 className="text-lg font-bold text-white mb-2">{metadata.title}</h2>
                ...
                <button
                    ...
                    className={`w-full p-3 rounded-md font-semibold transition duration-200 ${
                        loading ? 'bg-[#1DA1F2]/70 cursor-not-allowed' : 'bg-[#1DA1F2] hover:bg-[#1A91DA]'
                    }`}
                >
                ...
                <p className="text-gray-400 text-xs text-center mt-6">
                    By using this tool, you agree not to violate YouTube's Terms of Service.
                </p>
